...
$('#posMessagebtn').click( function() {		
	...
	function validate_post_new_task()
	{
		...
		//On Submitting
		form.submit(function(){
		if($('input[name=user_location_id[]]:checked').length >0)
		{
			if(validate_task_name() & validate_task_description() & validate_task_to_price() & validate_task_price())
				return true
			else
				return false;
		}
		else{
			if(validate_task_name() & validate_task_description() &validate_address1() &validate_zipcode() & validate_task_to_price() & validate_task_price())
				return true
			else
				return false;
		}
		...
		function validate_task_description()
		{
			...
			//if it's NOT valid
			else if(task_description.val().length < 15)
			{
				task_description.addClass("error1");
				//task_descriptionInfo.text("15 letters minimum required!");
				task_descriptionInfo.addClass("error1");
				task_description_symbol.removeClass("tick_mark");
				task_description_symbol.addClass("cross_mark");
				return false;
		...
		function validate_zipcode()
		{
			...
			if(zipcode.val()=='')
			{
				...
				//zipcodeInfo.text("Enter Post Code!");
			...
			else
			{
				...
				{
					zipcode.addClass("error1");
					//zipcodeInfo.text("Enter valid post code without any space.");
					zipcodeInfo.addClass("error1");
					zipcode_symbol.removeClass("tick_mark");
					zipcode_symbol.addClass("cross_mark");
					return false;
		...
		{		
			if(task_price.val()=='' || Math.floor(task_price.val()) < Math.floor(task_to_price.val()))
			{
			...
			else if(task_price.val() == 0 || task_price.val() == 0.00)
			{
				task_price.addClass("error1");
				//task_priceInfo.text("You can not put 0 here!");
				task_priceInfo.addClass("error1");
				task_price_symbol.removeClass("tick_mark");
				task_price_symbol.addClass("cross_mark");
				return false;
	...
	function validate_signup()
	{
		...
		var last_name_symbol = $("#full_name_symbol");
		...
		email.focus(function() {  
			...
			mobile_noTR.removeClass('field_main'); 
			cppasswordTR.removeClass('field_main'); 
		} );
		...
		cpassword.focus(function() {  
			cppasswordTR.addClass('field_main'); 
			passwordTR.removeClass('field_main'); 
			zip_codeTR.removeClass('field_main');
			emailTR.removeClass('field_main');	
			full_nameTR.removeClass('field_main');
		...
		cppassword.blur(validateCPassword);
		...
		cppassword.keyup(validateCPassword);
		
		
		
		//On Submitting
		form.submit(function(){
			if(validateFullName() & validateLastName() & validateEmail() & validatePassword() &validateCPassword() & validateMobile())
				return true
			else
				return false;
		});
	...
	function validateCPassword()
	{
		...
		if(cpassword.val()=='')
		{
			cpassword_symbol.removeClass("tick_mark");
			cpassword_symbol.removeClass("cross_mark");
			cpassword.addClass("error1");
			cpasswordInfo.text("Enter password!");
			cpasswordInfo.addClass("error1");
		...
		else
		{
			if(password.val()!=cpassword.val()){
				cpassword.addClass("error1");
				cpasswordInfo.text("Password and Confirm Password Not Same");
				cpasswordInfo.addClass("error1");
				cpassword_symbol.removeClass("tick_mark");
				cpassword_symbol.addClass("cross_mark");
				return false;
			}
			//it's valid
			else{			
				cpassword.removeClass("error1");
				//passwordInfo.text("Remember your password");
				cpasswordInfo.text("");
				cpasswordInfo.removeClass("error1");
				cpasswordInfo.addClass("success");
				cpassword_symbol.removeClass("cross_mark");
				cpassword_symbol.addClass("tick_mark");
				return true;
			}
		}
	}
	...
	{	
		...
		if(full_name.val()=='')
		{
			...
			full_nameInfo.text("Enter First name!");
	...
	{	
		...
		if(full_name.val()=='')
		{
			last_name_symbol.removeClass("tick_mark");
			last_name_symbol.removeClass("cross_mark");
		...
		//if it's NOT valid
			...
			last_name.addClass("error1");
			//full_nameInfo.text("We want names with more than 4 letters!");
			last_nameInfo.text("4 letters minimum required!");
			last_nameInfo.addClass("error1");
			last_name_symbol.removeClass("tick_mark");
			last_name_symbol.addClass("cross_mark");
			return false;
		}
		...
		//if it's valid
			...
			if(filter.test(a)){
				...
				last_name_symbol.removeClass("cross_mark");
				last_name_symbol.addClass("tick_mark");
				return true;
			}
			//if it's NOT valid
			else{
				last_name.addClass("error1");
				last_nameInfo.text("Enter valid full name.");
				last_nameInfo.addClass("error1");
				last_name_symbol.removeClass("tick_mark");
				last_name_symbol.addClass("cross_mark");
				return false;
	...
	function validateMobile()
	{
		...
		else*/ if(mobile_no.val().length <= 8 ||  mobile_no.val().length >=11)
		{		
			mobile_no.addClass("error1");
			mobile_noInfo.text("Enter valid 9 or 10 digit mobile number!");
			mobile_noInfo.addClass("error1");
	...
	function validateZipcode()
	{
		...
		if(zip_code.val()=='')
		{
			...
			zip_codeInfo.text("Enter post code!");
		...
		{		
			zip_code.addClass("error1");
			zip_codeInfo.text("4 characters minimum required!");
			zip_codeInfo.addClass("error1");
			zip_code_symbol.removeClass("tick_mark");
			...
			//if it's NOT valid
				...
				zip_code.addClass("error1");
				zip_codeInfo.text("Enter valid post code without any space.");
				zip_codeInfo.addClass("error1");
				zip_code_symbol.removeClass("tick_mark");
				zip_code_symbol.addClass("cross_mark");
				return false;
	...
	function validate_edit_account()
	{
		...
		var zip_code = $("#zip_code_E");
		var zip_codeInfo = $("#zip_codeInfo_E");
		var zip_codeTR = $("#zip_codeTR_E");
		...
		var email = $("#email_E");
		var emailInfo = $("#emailInfo_E");
		var emailTR = $("#emailTR_E");

(function ($) {
	$.widget("ui.rcarouselT",{
		_create: function() {
			...
			// create wrapperT inside root element; this is needed for animating
			$root
				.addClass("ui-carousel-top-vertical")
				.children()
				.wrapAll("<div class='wrapperT'></div>");
		...
		_addElement: function(jQueryElement,direction){
			var $root = $( this.element ),
				$content = $root.find("div.wrapperT"),
				options = this.options;
		...
		_saveElements: function(){
			var $el,
				$root = $( this.element ),
				$elements = $root.find("div.wrapperT").children(),
				data = $root.data("data");
		...
		_setCarouselHeight: function() {
			...
			if ( options.orientation === "vertical" ) {
				_newHeight = options.visible * options.height + options.margin * (options.visible - 1)+16;
			} else {
		...
		options: {
			...
			startAtPage: 0,
			navigation: {
				next: "#ui-carousel-next-top-vertical",
				prev: "#ui-carousel-prev-top-vertical"
			}
		}
	});
}(jQuery));
(function ($) {
	$.widget("ui.rcarousel", {
		_create: function() {
			...
			$root
				.addClass("ui-carousel-cat-fancy")
				.children()
...
var $root = $( this.element ),
				$content = $root.find("div.wrapper"),
				options = this.options;//alert(data.paths.length);
					for(i=0;i<data.paths.length;i++)
					{
										if(selcatid!=0)		
									{//alert(data.paths[i].attr('id'));
											if(selcatid==data.paths[i].attr('id'))
											{
											
												//alert(data.paths[i].attr('id'));
												// selcatid=$(this).attr('id');
												//$(this).removeClass("slidecatfancy");
												data.paths[i].removeClass("deselected");
												data.paths[i].addClass("selected");
											//	alert(data.paths[i].attr('class'));
												
											}
											 else
											  {
											 data.paths[i].removeClass("slidecatfancy");
											 data.paths[i].removeClass("selected");
											  data.paths[i].addClass("deselected");
											  }	
									}
						//alert(data.paths[i].attr('id'));//alert(selcatid);
					}
			if ( !data.animated ) {
				data.animated = true;
	...
	var $root = $( this.element ),
		...
		options: {
			...
			startAtPage: 0,
			navigation: {
				next: "#ui-carousel-next-cat-fancy",
				prev: "#ui-carousel-prev-cat-fancy"
			}
		}
	});
}(jQuery));
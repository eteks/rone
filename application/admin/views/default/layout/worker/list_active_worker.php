<script type="text/javascript" language="javascript">
	...
	function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>worker/list_active_worker/'+limit;
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>worker/list_active_worker/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
	...
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>worker/list_active_worker';
		}
	}
...
<div id="content">        
	<?php if($msg != ""){
            ...
            if($msg == "delete"){ $error = 'Record has been deleted Successfully.';}
			 if($msg == "active"){ $error = 'Runner has been Successfully activated.';}
    ?>
	...
	<div class="box">		
		...
		<h2 class="box-header">Active Workers </h2>
			...
			<table class="tablebox">
               ...
               <form  name="frm_active" id="frm_active" action="<?php echo base_url();?>worker/action_worker/" method="post">
              <input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
                <input type="hidden" name="action" id="action" />
                <input type="hidden" name="fname" value="list_active_worker"/>
               
				
               <?php echo anchor('worker/export_active_worker/'.strtotime(date('H:i:s')),'Export CSV','class="button white" style="margin: 0px;"'); ?>
                
                 <a href="javascript:void(0)"  onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'frm_active')" class="button white" style="margin: 0px;"><span class="icon_text cancel"></span> Delete</a>
				...
				<?php
                  ...
                  ?>
					<tr class="<?php echo $cl; ?>">
						...
						<td><?php echo $site_setting->currency_symbol.$this->worker_model->get_sum_received_amount($row->user_id); ?></td>
						<td><?php echo $site_setting->currency_symbol.$this->worker_model->get_sum_fees_paid($row->user_id); ?></td>

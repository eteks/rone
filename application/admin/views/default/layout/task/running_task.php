...
function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>task/running_task/'+limit;
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>task/running_task/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
...
<div id="content">        
	<?php if($msg != ""){
			 ...
			 if($msg == "suspend"){ $error = 'Record has been suspended Successfully.';}
    ?>
	...
	<div class="box">		
		...
		<h2 class="box-header">Assigned Tasks </h2>
			...
			<table class="tablebox">
               ...
               <form name="frm_runningtask" id="frm_runningtask" action="<?php echo base_url();?>task/action_task/" method="post">
              <input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
                <input type="hidden" name="action" id="action" />
				
                 <a href="#" class="button white" style="cursor:default;">Total Escrow Amount : <?php echo $site_setting->currency_symbol.$this->task_model->escrow_amount();?></a>
                 
               <?php echo anchor('task/export_running_task/'.strtotime(date('H:i:s')),'Export CSV','class="button white" style="margin: 0px;"'); ?>
                  
                 
                 
				 
				 <a href="javascript:void(0)"  onclick="setaction('chk[]','suspend', 'Are you sure, you want to Supspend selected task(s)?', 'frm_runningtask')" class="button white" style="margin: 0px;"><span class="icon_text accept"></span> Suspend</a>
                 
                 <a href="javascript:void(0)"  onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'frm_runningtask')" class="button white" style="margin: 0px;"><span class="icon_text cancel"></span> Delete</a>
				...
				<?php
                  ...
                  ?>
                    ...
                    <td><input type="checkbox" name="chk[]" id="chk" value="<?php echo $row->task_id;?>" /></td>
                        ...
                        <td><?php if($row->task_activity_status =="1"){ echo "Assigned"; }else{ echo "Not Assigned"; } ?></td>
				  ...
				  <?php
                            $i++;
								  ...
								  <tr class="alter">
                                    <td colspan="15" align="center" valign="middle" height="30">No Task has been added yet.</td></tr>

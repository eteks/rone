<script type="text/javascript" language="javascript">
	function gomain(x)
	{
		...
		if(x == 'all')
		{
			window.location.href= '<?php echo base_url();?>task/post_task';
		}
	}
...
function getlimit(limit)
	{
		...
		{
			window.location.href='<?php echo base_url();?>task/post_task/'+limit;
		}
	...
	function getsearchlimit(limit)
	{
		...
		else
		{
			...
			window.location.href='<?php echo base_url();?>task/post_task/'+limit+'/<?php echo $option.'/'.$keyword; ?>';
		}
...
<div id="content">        
	...
	<div class="box">		
		...
		<h2 class="box-header">Posted Tasks </h2>
			...
			<table class="tablebox">
               ...
               <form name="frm_opentask" id="frm_opentask" action="<?php echo base_url();?>task/action_task/" method="post">
              <input type="hidden" name="offset" id="offset" value="<?php echo $offset; ?>" />
                <input type="hidden" name="action" id="action" />
				
               <?php echo anchor('task/export_post_task/'.strtotime(date('H:i:s')),'Export CSV','class="button white" style="margin: 0px;"'); ?>
                  
                  <a href="javascript:void(0)"  onclick="setaction('chk[]','suspend', 'Are you sure, you want to Supspend selected task(s)?', 'frm_opentask')" class="button white" style="margin: 0px;"><span class="icon_text accept"></span> Suspend</a>
                 <a href="javascript:void(0)"  onclick="setaction('chk[]','delete', 'Are you sure, you want to delete selected record(s)?', 'frm_opentask')" class="button white" style="margin: 0px;"><span class="icon_text cancel"></span> Delete</a>

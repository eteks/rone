...
class Category_model extends CI_Model 
{
	...
	*/
	...
	function get_category_info($name){
		...
		if($pid == 0){ 
			...
			$query = $this->db->get_where("task_category",array('task_category_id'=>$category_info->task_category_id));
			$category_info = $query->row();
			return $category_info;
		}
	...
	*/
	...
	function prentcat_details($id){
		$query = $this->db->get_where("task_category",array('task_category_id'=>$id));
		return $query->row();
	}

	function browse_all_subcategory($cat_id)
	{

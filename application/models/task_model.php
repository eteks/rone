...
class Task_model extends CI_Model 
{
	...
	*/
	...
	function cat_list()
	{
		...
		$query = $this->db->query("select * from task_category");
	...
	*/
	...
	function add_worker_offer($task_id)
	{
	  ...
	  $this->db->insert('message', $message);
		...
		{
			$bidreduse =$this->bid_count_reduse(get_authenticateUserID());
		}
	...
	*/
	...
	function save_step_zero($task_id = ''){
		...
		if($this->input->post('sub_step') != ''){
			...
			if($this->input->post('task_invit_worker')) { $task_invit_worker = $this->input->post('task_invit_worker'); }else { $task_invit_worker =0; }
			//echo $task_id;
			if($task_id !='' && $task_id !=0){
				...
				'tsk_invit_worker_id'=>$task_detail->tsk_invit_worker_id,
				...
				$data= array(
					...
					'user_id' => get_authenticateUserID(),	
					'task_auto_assignment' => $autoassign,
					'tsk_invit_worker_id'=>$task_invit_worker,
					'task_repeat' =>$repeat,
					'task_repeat_week' =>$repeat_week,
					'task_assing_worker'=>$worker,
					'task_post_date' =>$post_date,
			...
			{ 
				...
				if($this->input->post('address1')!=''  && $this->input->post('zipcode')!='')
				{
				$data_location=array(
					'task_id'=>$task_id,				
	...
	*/
	...
	function traf_invitetion($id)
	{

		//echo $id; exit;
		$sql="select * from trc_task where task_id='".$id."'";
		$recordSet=$this->db->query($sql);
		$row = $recordSet->row_array();

		if($row['done_online']==1)
		{
	...
	*/
	...
	function save_step_one($data)
	{
		...
		$swword = array("ä", "å", "é","ö");
		$engword   = array("a", "a", "e","o");
	...
	*/
	...
	function getCityList($sate_id='')
	{
		$addsql="";
			if($sate_id!=''){
				$addsql = " and `state_id`='".$sate_id."' order by city_name ";
			}
	...
	*/
	...
	function get_category_task_list($category_id)
	{
		...
		{				
		$query = $this->db->query("select * from ".$this->db->dbprefix('task')." tk, ".$this->db->dbprefix('user')." ur, ".$this->db->dbprefix('user_profile')." up where tk.task_category_id='".$category_id."' and tk.user_id = ur.user_id and ur.user_id = up.user_id and (tk.task_is_private=1 or tk.task_is_private=0) and tk.task_status = 1");
		
		}
		else
		{		
			$query = $this->db->query("select * from ".$this->db->dbprefix('task')." tk, ".$this->db->dbprefix('user')." ur, ".$this->db->dbprefix('user_profile')." up where tk.task_category_id='".$category_id."' and tk.user_id = ur.user_id and ur.user_id = up.user_id and tk.task_status = 1 and tk.task_is_private=0 order by tk.task_id desc");
	...
	function get_fullall_task_list()
	{
		...
		if($user_status=='worker')
		{				
		$query = $this->db->query("select * from trc_task WHERE task_status='1' order by task_id desc");
	...
	//   get data from table  trc_task after search in ajax
	function get_ajax_task_data($data)
	{
		...
		$sortsql="";
		if($task_title !=''){
			$addsql.=" and task_name LIKE '%".$task_title."%' ";
		}
		if($task_status !=''){
			$addsql.=" and task_activity_status = '0' ";
		}
		if($task_type !='' && $task_type == 'online_task'){
			$addsql.=" and done_online = '1' ";
		}
		if($cat_name !=''){
			$addsql.=" and task_category_id = '".$cat_name."'";
		}
		if($task_status !='' && $task_type == 'task_loc'){
			$addsql.=" and task_city_id > 0 ";
		}
		if($location_name !=''){
			$addsql.=" and task_city_id = '".$location_name."' ";
			/*$sql1="SELECT city_id FROM trc_city WHERE city_name like '".$location_name."%' limit 1";
			$recordSet1=$this->db->query($sql1);
            if($recordSet1->num_rows()>0){
             	$row =  $recordSet1->row_array();
				$city_id = $row['city_id'];
				if($city_id !=''){
					$addsql.=" and task_city_id = '".$city_id."' ";
				}
            }*/
		}
		if($sort_by !=''){
			$sortsql .=" order by $sort_by ";
		}
		
		
		$sql="select * from trc_task where 1 $addsql $sortsql";
		$recordSet=$this->db->query($sql);
	...
	{
		$numbid=$this->getNameTable("trc_user","avilable_bid","user_id",$userid);

		$newnumber=$numbid-2;

		//echo $newnumber;exit;

		$this->db->query("update ".$this->db->dbprefix('user')." set avilable_bid='".$newnumber."' where user_id='".$userid."'");
	}

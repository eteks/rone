...
class User_model extends CI_Model 
{
	...
	*/
	...
	function change_password()
	{
		...
		$this->session->set_flashdata('passwordchange', 'Current Password not match');
			   ...
			   $this->upload->initialize($config);
					...
					else
					{
						...
						if($this->input->post('prev_image')!='')
						{
							...
							if(file_exists(base_path().'upload/user/'.$this->input->post('prev_image')))
							{
								$link2=base_path().'upload/user/'.$this->input->post('prev_image');
								unlink($link2);
							}						
	...
	*/
	...
	function get_employer_recent_reviews($user_id,$reviews_limit)
	{
		
		$this->db->select('*');
		$this->db->from('trc_employer_review cm');
		$this->db->join('task tk','cm.task_id=tk.task_id');
		$this->db->join('user us','cm.employer_id=us.user_id');
		$this->db->join('user_profile up','us.user_id=up.user_id');
		$this->db->where('cm.employer_id',$user_id);
	...
	*/
	...
	function get_employe_total_reviews($user_id)
	{
		
		$this->db->select('*');
		$this->db->from('employer_review cm');
		$this->db->join('task tk','cm.task_id=tk.task_id');
		$this->db->join('user us','cm.employer_id=us.user_id');
		$this->db->join('user_profile up','us.user_id=up.user_id');
		$this->db->where('cm.employer_id',$user_id);
	...
	*/
	...
	function get_emp_all_reviews($user_id,$limit,$offset)
	{
	...
	*/
	...
	function get_worker_city_cat_wisetasklist($user_id)
	{
		$sql="SELECT distinct(`taskid`)  FROM `worker_invitetion` WHERE is_show='1'";
		$res=$this->db->query($sql);
			if ($res->num_rows() > 0) {
			foreach ($res->result_array() as $row){
			$sql="SELECT * FROM `trc_task` WHERE task_id='".$row['taskid']."' AND user_id!='".$user_id."' AND task_status='1' AND task_activity_status='0' ";
			$recordSet=$this->db->query($sql);
				if ($recordSet->num_rows() > 0) {
					$rsn[]=$recordSet->result_array();
				}
	...
	function get_reject_offer($data)
	{
		$sql="UPDATE worker_invitetion SET is_show='0' WHERE taskid='".$data['taskid']."' AND userid='".$data['userid']."'";
		$this->db->query($sql);
	}
	function get_accept_offer($data)
	{
		$sql="UPDATE worker_invitetion SET is_show='0' WHERE taskid='".$data['taskid']."' AND userid='".$data['userid']."'";
		$this->db->query($sql);
	}

	function get_activepostaccount($user_id)
	{
			$sql="UPDATE trc_user SET profile_active='1' WHERE user_id='".$user_id."'";
			$this->db->query($sql);

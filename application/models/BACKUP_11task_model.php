...
class Task_model extends CI_Model 
{
	...
	*/
	...
	function traf_invitetion($id)
	{
		...
		$query ="SELECT distinct(trc_worker.worker_id) FROM (`trc_worker`) JOIN `trc_user` ON `trc_worker`.`user_id`=`trc_user`.`user_id` JOIN `trc_user_profile` ON `trc_worker`.`user_id`=`trc_user_profile`.`user_id` JOIN `trc_worker_cities` ON `trc_worker`.`user_id`=`trc_worker_cities`.`user_id` WHERE `trc_worker`.`worker_app_approved` = 1 AND `trc_worker`.`worker_status` = 1  AND FIND_IN_SET('".$row['task_category_id']."',trc_worker.worker_task_type) ORDER BY `trc_worker`.`worker_level` desc";
		$res=$this->db->query($query);
		foreach ($res->result() as $row)
		{
			$sqlins="INSERT INTO worker_invitetion SET workerid='".$row->worker_id."',taskid='".$id."',is_show='1'";

...
class User_task_model extends CI_Model 
{
	...
	*/
	...
	function pay_now($task_id,$worker_id)
	{
	...
	*/
	...
	/*
	Function name :get_total_draft_task()
	Parameter : none
	Return : integer all draft task count
	Use : user total number of draft tasks
	*/
	
	function get_total_draft_task()
	{
		...
		$this->db->join('user','task.user_id=user.user_id');
		$this->db->where('user.user_id',get_authenticateUserID());
		$this->db->where('task.task_status',2);
		$this->db->where('task.task_activity_status',0);
		$this->db->order_by('task.task_id desc');
		$query = $this->db->get();
	...
	/*
	Function name :get_draft_task_list()
	Parameter : $limit (for paging), $offset (for paging)
	Return : array of all draft tasks
	Use : user all draft tasks
	*/
	
	function get_draft_task_list($limit,$offset)
	{
	...
	*/
	...
	function employer_review($data)
	{
			$sql="INSERT INTO trc_employer_review SET comment='".$data['comment']."',task_id='".$data['task_id']."',employer_id='".$data['employer_id']."',comment_rate='".$data['comment_rate']."'";
		    $row=$this->db->query($sql);
	}
	function worker_id($wid)
	{
		$query=$this->db->query("SELECT `user_id` FROM `trc_worker` WHERE `worker_id`='".$wid."'");
		if($query->num_rows>0) 
		{
			return $query->row();

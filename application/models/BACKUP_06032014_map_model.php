...
class Map_model extends CI_Model 
{
	...
	*/
	function get_city_alltask()
	{
			...
			$query = $this->db->query("select * from ".$this->db->dbprefix('task')." tsk, ".$this->db->dbprefix('city')." cty where tsk.task_city_id = cty.city_id and  tsk.task_status=1 and tsk.task_activity_status!=4");
			...
			else
			{
						...
						$query = $this->db->query("select * from ".$this->db->dbprefix('task')." tsk, ".$this->db->dbprefix('city')." cty where tsk.task_city_id = cty.city_id and  tsk.task_status=1 and tsk.task_is_private=0 and tsk.task_activity_status!=4");
	...
	*/
	function get_city_task($city)
	{
		...
		$query = $this->db->query("select * from ".$this->db->dbprefix('task')." tsk , ".$this->db->dbprefix('city')." cty where  tsk.task_city_id = cty.city_id and tsk.task_status=1 and tsk.task_activity_status!=4 and cty.city_name like '%".$city."%'");
			
			}
			else
			{
			
					$query = $this->db->query("select * from ".$this->db->dbprefix('task')." tsk , ".$this->db->dbprefix('city')." cty where  tsk.task_city_id = cty.city_id and tsk.task_status=1 and tsk.task_activity_status!=4 and tsk.task_is_private=0 and cty.city_name like '%".$city."%'");

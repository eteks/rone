...
class Worker extends ROCKERS_Controller 
{
	...
	*/
	...
	function edit()
	{
		...
		$this->form_validation->set_rules('worker_city', 'City', 'required|alpha_space');
		$this->form_validation->set_rules('worker_state', 'State', 'required|alpha_space');
		$this->form_validation->set_rules('worker_zipcode', 'Postal Code', 'required|alpha_numeric|min_length['.$site_setting->zipcode_min.']|max_length['.$site_setting->zipcode_max.']');
		...
		$this->form_validation->set_rules('worker_background', 'Background check,', 'required');
			...
			if($this->form_validation->run() == FALSE){
				...
				else
				{
					...
					$data['worker_hear_about'] = $ex_worker_hear_about;
	...
	*/
	...
	{	
		...
		$pageTitle='AskForTask Application - '.$user_info->profile_name.' - '.$meta_setting->title;
		$metaDescription='AskForTask Application - '.$meta_setting->meta_description;
		$metaKeyword='AskForTask Application - '.$meta_setting->meta_keyword;

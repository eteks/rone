	...
	function buy_credit()
	{
		...
		if($this->form_validation->run() == FALSE || $amount_error=='fail' || $own_amount_error=='fail'){			
				...
				} else
				{
					...
					$_FILES['bank_slip_name']['name']     =   $_FILES['bank_slip']['name'];
					$_FILES['bank_slip_name']['type']     =   $_FILES['bank_slip']['type'];
					$_FILES['bank_slip_name']['tmp_name'] =   $_FILES['bank_slip']['tmp_name'];
					$_FILES['bank_slip_name']['error']    =   $_FILES['bank_slip']['error'];
					$_FILES['bank_slip_name']['size']     =   $_FILES['v']['size'];	
					$file_name=get_authenticateUserID().$rand.$_FILES['bank_slip_name']['name'];	

					$config =  array(
									  'upload_path'     => '/home/saivisiontech/public_html/newsite/upload/bank_doc',
									  'file_name'		=>$file_name,
									  'allowed_types'   => "*",
									  'overwrite'       => TRUE
					 );
					...
					else
					{
						...
						echo "<a href='http://saivisiontech.com/newsite/dashboard' class='btn btn-default'>Go back</a>";

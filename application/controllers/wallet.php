...
class Wallet extends ROCKERS_Controller 
{
	...
	*/
	...
	{	
		...
		{	
			...
			{	
				...
				//var_dump($pay);exit;
				//redirect('/wallet/pay_strip/'.$total);
				//$this->($amount);		
	...
	function buycredit(){
		...
		$gateway_id='1';
		$pay=$this->wallet_model->get_paymentid_result($gateway_id);
		redirect('/wallet/'.trim($pay->function_name).'/'.$pay->id.'/'.$total.'/0/0/'.$creditid);
	...
	*/
	...
	function auth_payu($id,$amt,$task_id=0,$task_comment_id=0)
	{
			...
			*/
			...
			/**/
				...
				$setTransactionArray['AdditionalInformation']['supportedPaymentMethods'] = 'CREDITCARD,WALLET,EFT';//
				
				$setTransactionArray['Basket']['description'] = "Add Amount to wallet";
				$setTransactionArray['Basket']['amountInCents'] = $total*100;//;
				...
				$setTransactionArray['Customer']['countryCode'] = 27;//$site_setting['currency_code'];
				...
				if(isset($doAutoRedirectToPaymentPage) && ($doAutoRedirectToPaymentPage == 1) ) {
					if( (isset($returnData['return']['successful']) && ($returnData['return']['successful'] === true) && isset($returnData['return']['payUReference']) ) ) {			
						$_SESSION["payUReference"] = $returnData['return']['payUReference']; // webxtor
						$_SESSION["id"] = $id; 
						//Redirecting to payment page
						//print_r($_SESSION); echo $this->db->insert_id();//exit;
	...
	{	
		...
		// Payu EFT double redirect bug
		if (isset($_SESSION["msg"][$_SESSION["add_wallet"]])) {
			$msg = $_SESSION["msg"][$_SESSION["add_wallet"]];
			redirect('wallet/index/0/'.$msg);
		}

		if (!$_SESSION["id"] || !isset($_SESSION["add_wallet"]) || !isset($_GET['PayUReference']) || $_GET['PayUReference'] != $_SESSION["payUReference"]) {
		...
		*/
		...
		if(is_object($soap_client) && false) {    
			...
			var_dump($soapCallResult->return); 
			//exit;        
		} 
		
		$gateway_details=get_payment_gateway_details_by_id($id);

		//echo $soapCallResult->return->successful ? 1 : 0;	exit;
		...
		if ($soapCallResult->return->successful && $soapCallResult->return->transactionState == 'SUCCESSFUL') {
			$admin_status='Confirm';
			$_SESSION["msg"][$_SESSION["add_wallet"]] = $msg;
			//unset($_SESSION["add_wallet"]);
		} else if ($soapCallResult->return->successful && $soapCallResult->return->transactionState == 'AWAITING_PAYMENT') {
			$msg= "review";
			$_SESSION["msg"][$_SESSION["add_wallet"]] = $msg;
			//unset($_SESSION["add_wallet"]);
			redirect('wallet/index/0/'.$msg);
	...
	*/
	...
	function paypal($id,$amt,$task_id=0,$task_comment_id=0,$taskcredit=0)
	{
			...
			$this->paypal_lib->add_field('return', site_url('wallet/paypalsuccess/'.$task_id.'/'.$task_comment_id.'/'.$taskcredit));
	...
	*/
	...
	function paypalsuccess($task_id=0,$task_comment_id=0,$taskcredit=0)
	{
			...
			$msg= "add";
				...
				elseif($taskcredit){
				  ...
				  $bid=$this->wallet_model->getNameTable("trc_membership","numbid","id",$taskcredit);
				  $avbid=$this->wallet_model->getNameTable("trc_user","avilable_bid","user_id",get_authenticateUserID());
				  $newbid=$avbid+$bid;
				  $user_info = $this->user_model->get_user_info(get_authenticateUserID());
					...
					$datanew = array(	
						...
						'phone_number'=>$user_info->mobile_no,
						'pack_id'=>$taskcredit,
					);
					$this->db->insert('online_credit', $datanew); 
	...
	*/
	...
	function withdraw_wallet()
	{
		...
		if($this->input->post('withdraw_method')=='bank')
		{
			...
			$this->form_validation->set_rules('bank_account_holder_name', 'National Identity Document Number', 'required');			
			$this->form_validation->set_rules('bank_account_number', 'Address', 'required');
			$this->form_validation->set_rules('bank_branch', 'Mobile Number', 'required');
			$this->form_validation->set_rules('bank_ifsc_code', 'Email Id', 'required|valid_email');
		...
		if($this->input->post('withdraw_method')=='gateway')
		{
			...
			$this->form_validation->set_rules('gateway_name', 'Enter Your Paypal Email id', 'required|valid_email');
	...
	function buy_credit()
	{
		...
		if($this->input->post('withdraw_method')=='bank')
		{
			...
			$this->form_validation->set_rules('bank_account_holder_name', 'National Identity Document Number ', 'required');			
			$this->form_validation->set_rules('bank_account_number', 'Address', 'required');
			$this->form_validation->set_rules('bank_branch', 'Mobile Number', 'required|numeric');
			$this->form_validation->set_rules('bank_ifsc_code', 'Email', 'required|valid_email');
		...
		if($this->form_validation->run() == FALSE || $amount_error=='fail' || $own_amount_error=='fail'){			
				...
				} else
				{
					...
					$data['bank_account_holder_name']=$this->input->post('bank_account_holder_name');
					$data['bank_account_number']=$this->input->post('bank_account_number');
					$data['packid']=$this->input->post('packid');
		       ...
		       $theme = getThemeName();
				...
				$this->template->write('metaKeyword',$metaKeyword,TRUE);
				$this->template->write_view('header',$theme .'/layout/common/header_login',$data,TRUE);
				$this->template->write_view('content_center',$theme .'/layout/wallet/buy_credit',$data,TRUE);
				$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
				$this->template->render();	
			...
			{	
				...
				{				
					$this->wallet_model->add_credit_request();				
					$msg='success';
				}
					redirect('wallet/thankyoubankpay');		
	...
	*/
	...
	function strippay()
    {
		...
		$pageTitle='Online Payment - '.$meta_setting->title;
		$metaDescription='Online Payment - '.$meta_setting->meta_description;
		$metaKeyword='Online Payment - '.$meta_setting->meta_keyword;
    ...
    function checkout()
	{
			...
			\Stripe\Stripe::setApiKey("sk_test_FXEIDXJOKa0pinicdAbN4skB"); //Replace with your Secret Key
 
			try {
			$charge = \Stripe\Charge::create(array(
				"amount" => $this->input->post('price_new'),
				"currency" => "cop",
  ...
  // Something else happened, completely unrelated to Stripe
			...
			$pckid=$this->input->post('packid');

			$numbid1=$this->wallet_model->getNameTable("trc_membership","numbid","id",$pckid);
			...
			$this->db->insert('online_credit',$data_gateway);	

			$numbid=$numbid1+$user_info->avilable_bid;

			$this->db->query("update ".$this->db->dbprefix('user')." set avilable_bid='".$numbid."' where user_id='".get_authenticateUserID()."'");
			...
			$this->template->write('metaKeyword',$metaKeyword,TRUE);
			$this->template->write_view('header',$theme .'/layout/common/header_login',$data,TRUE);
			$this->template->write_view('content_center',$theme .'/layout/wallet/thankyoupay',$data,TRUE);
			$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
			$this->template->render();	
	...
	function thankyoubankpay()
	{
			...
			$pageTitle='Payment thankyou - '.$meta_setting->title;
			$metaDescription='Payment thankyou- '.$meta_setting->meta_description;
			$metaKeyword='Payment thankyou- '.$meta_setting->meta_keyword;
			
			$this->template->write('pageTitle',$pageTitle,TRUE);
			$this->template->write('metaDescription',$metaDescription,TRUE);
			$this->template->write('metaKeyword',$metaKeyword,TRUE);
			$this->template->write_view('header',$theme .'/layout/common/header_login',$data,TRUE);
			$this->template->write_view('content_center',$theme .'/layout/wallet/thankyoubankpay',$data,TRUE);
			$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
			$this->template->render();	
	}

	function stripdeposit()
    {
        $theme = getThemeName();
		$this->template->set_master_template($theme .'/template.php');
		...
		$this->template->write('metaKeyword',$metaKeyword,TRUE);
		$this->template->write_view('header',$theme .'/layout/common/header_login',$data,TRUE);
		$this->template->write_view('content_center',$theme .'/layout/wallet/stripdeposit',$data,TRUE);
		$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
		$this->template->render();	
    }

	function pay_strip($id,$task_id=0,$task_comment_id=0)
	{
		...
		$this->template->write('metaKeyword',$metaKeyword,TRUE);
		$this->template->write_view('header',$theme .'/layout/common/header_login',$data,TRUE);
		$this->template->write_view('content_center',$theme .'/layout/wallet/thankyoustrip',$data,TRUE);
		$this->template->write_view('footer',$theme .'/layout/common/footer',$data,TRUE);
		$this->template->render();	

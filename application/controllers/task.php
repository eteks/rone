...
class Task extends ROCKERS_Controller {
	...
	*/
	...
	{
		redirect('task/all_task');
	}
	
	function task_ajax_data(){ // get search result from left menu setting in ajax
		if($_POST)
		{
			...
			$this->session->set_userdata('task_title', $data['task_title']);
			$this->session->set_userdata('task_status', $data['task_status']);
			$this->session->set_userdata('task_type', $data['task_type']);
			$this->session->set_userdata('sort_by', $data['sort_by']);
			$this->session->set_userdata('cat_name', $data['cat_name']);
			$this->session->set_userdata('location_name', $data['location_name']);
		}
		$data['task_title'] = $this->session->userdata('task_title');
...
<div class="task-list-left-ptd">
	...
	<div class="post-task-list-ptd">
		<ul>';
			foreach($task_detail as  $kk=>$vv){
			 $city        =get_cityDetail($vv->task_city_id);
			 $user_detail=$this->user_model->get_user_profile_by_id($vv->user_id);
			...
			$html.='
			<li>
				<a href="'.base_url().'tasks/'.$vv->task_url_name.'">
					<div class="post-task1-ptd">
						<div class="post-tasker-img-ptd"><img src="'.base_url().$user_image_new.'" width="72" height="72" alt="" style="border-radius:5px;"/></div>
						<div class="post-task-info-ptd">
							<div class="post-task-info-left-ptd">
								<h2>'.$vv->task_name.'</h2>
								<p><img src="'.base_url().getThemeName().'/images/pin_icon_pt.png" alt="" />'.$city->city_name.'</p>
							</div>
							<div class="post-task-price-ptd">
								<div class="post-task-budget-ptd">$ '.$vv->task_to_price.'</div>
...
echo $html; exit;
	...
	function step_one($task_id)
	{
		...
		if($this->form_validation->run() == FALSE || $day_error!=''){
			...
			if($_POST)
			{
				...
				$_FILES['userfile']['type']     =   $_FILES['sheet_attachment']['type'];
				$_FILES['userfile']['tmp_name'] =   $_FILES['sheet_attachment']['tmp_name'];
				...
				$_FILES['taskfile']['name']     =   $_FILES['task_img']['name'];
				$_FILES['taskfile']['type']     =   $_FILES['task_img']['type'];
				$_FILES['taskfile']['tmp_name'] =   $_FILES['task_img']['tmp_name'];
			...
			else
			{
				...
				$data['task_category_id'] = $task_detail->task_category_id;
			...
			if($_FILES['task_img']['name']!="")
			{
					...
					if(in_array($ext,$allowed) ) {
						if( move_uploaded_file($_FILES['taskfile']['tmp_name'], "$uploads_dir/$file_name1"))
						{
	...
	*/
	...
	function done($task_id)
	{
		...
		if($task_detail->tsk_invit_worker_id!=''){
	...
	function relatedworker($city='', $offset=0)
	{
		...
		$data['day_task']=$this->input->post('day_task');
		$data['taskcity']=$this->input->post('taskcity');
		//print_r($data);	exit;
		...
		$data['cityname'] ='';
			...
			$config['total_rows'] = $this->worker_model->get_relatedtotal_taskers($city_id,$data);
			$config['per_page'] = $limit;
			
			$this->pagination->initialize($config);
			
			$data['page_link'] = $this->pagination->create_links();
			$data['taskers'] =$this->worker_model->get_related_taskers($city_id,$offset,$limit,$data);
		...
		$this->template->write('metaKeyword', $metaKeyword, TRUE);
		$this->template->write_view('header', $theme . '/layout/common/header_login', $data, TRUE);
		$this->template->write_view('content_center', $theme . '/layout/task/relatedworker', $data, TRUE);
		$this->template->write_view('footer', $theme . '/layout/common/footer', $data, TRUE);
		$this->template->render();
	
	}
	
	function alltasker($city='', $offset=0)
	{
		...
		$data['cityname'] ='';
			...
			$config['base_url'] = site_url('taskers/'.$city.'/');
			$config['total_rows'] = $this->worker_model->get_allrelatedtotal_taskers($city_id,$data);
			$config['per_page'] = $limit;
			
			$this->pagination->initialize($config);
			
			$data['page_link'] = $this->pagination->create_links();
			$data['taskers'] =$this->worker_model->get_allrelated_taskers($city_id,$offset,$limit,$data);
			$data['total_rows']=$config['total_rows'];
		...
		$this->template->write('metaKeyword', $metaKeyword, TRUE);
		$this->template->write_view('header', $theme . '/layout/common/header_login', $data, TRUE);
		$this->template->write_view('content_center', $theme . '/layout/task/alltasker', $data, TRUE);
		$this->template->write_view('footer', $theme . '/layout/common/footer', $data, TRUE);
		$this->template->render();
	
	}
	
	function invite($ref_link='')
	{
		...
		$data['taskmail']=$this->input->post('taskmail');	
		$data['task_id'] = $this->uri->segment(3);
		$data['user_id'] = $this->uri->segment(4);
		$task_detail=$this->task_model->get_task_detail($data['task_id']);
		$data['task_detail']=$task_detail;
		$data['task_taskname']=$task_detail->task_name;
		...
		$user_detail = $this->user_model->get_user_info(get_authenticateUserID());
		$poster_link = '<a href="'.base_url().'user/'.$user_detail->profile_name.'">'.ucfirst($user_detail->full_name).'</a>';	
		$wordkeremailaddress=$this->db->query("SELECT b.email FROM `trc_worker` a, trc_user b WHERE a.`user_id`=b.`user_id` and a.`worker_id`='".$data['userid']."'");
		...
		$email_message='Hi<br><br>Congratulation  , You have been invited to place an offer on '.$task_link.' By '.$poster_link.'<br><br><br><br>Task-IT Team<br><br>Thanks';
...
//		exit;
		...
		$this->load->view( $theme . '/layout/task/invite', $data);
	...
	*/
	...
	function task_city(){
        ...
        if(isset($_POST)){
            ...
            $output = $this->task_model->getCityList($_POST['state_id']);
        }

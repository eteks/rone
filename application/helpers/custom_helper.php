<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	***/
	...
	{
		$CI =& get_instance();
		$query = $CI->db->get_where("trc_user",array('user_id'=>$id));
	...
	***/
	...
	function getwallet_amount()
	{
		$CI =& get_instance();
		
		$query = $CI->db->query("SELECT SUM(debit) as sumd,SUM(credit) as sumc FROM trc_wallet where admin_status='Confirm' and user_id='".get_authenticateUserID()."'"); 
	...
	***/
	...
	function get_user_unread_notification()
	{
		...
		//$query = $CI->db->query("select * from ".$this->db->dbprefix('message')." m, ".$this->db->dbprefix('task')." t, ".$this->db->dbprefix('user')." u where m.receiver_user_id = '".get_authenticateUserID()."' and m.is_read=0 and m.task_id = t.task_id and m.receiver_user_id = u.user_id");
		if($query->num_rows()>0)
		{
			return $query->num_rows();	
		}

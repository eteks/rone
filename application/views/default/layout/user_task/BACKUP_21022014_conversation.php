...
<div id="s1postJ" class="padB10">Conversation with: <?php echo anchor('user/'.$check_worker_detail->profile_name,ucfirst($check_worker_detail->first_name).' '.ucfirst(substr($check_worker_detail->last_name,0,1)),' class="dhan" ');?></div>
...
<div class="offerbg" style="width:auto;">Offer Price:<b>
	<?php 
	    //$worker_id =
            ...
            <?php } else { ?>
              ...
              <input type="hidden" id="task_user" name="task_user" class="chbg fl" value="<?php echo $task_user;?>">
			   ...
			   if(($task_detail->task_activity_status==1 || $task_detail->task_activity_status==2) || (($task_detail->worker_agree==0 || $task_detail->worker_agree==1) && $task_detail->task_activity_status==3) ) { ?>
			     ...
			     <input type="button" id="complete" name="complete" class="chbg fl" value="Complete Task" onclick="window.location.href='<?php echo site_url('user_task/complete/'.$task_id) ?>'">
                 
                 
                   
			   
			<?php  }   if(($task_detail->task_activity_status==1) || (($task_detail->worker_agree==0 || $task_detail->worker_agree==1) && $task_detail->task_activity_status==2) ) { ?>
            
          <input type="button" id="complete" name="complete" class="chbg fl marL5" value="Dispute Task" onclick="window.location.href='<?php echo site_url('dispute/dispute_task/'.$task_id) ?>'">
              
              <?php }  ?>


<?php if($str!='') { ?> <p> <?php echo  $str; ?> Remaining to Complete or Dispute Task manually.</p> <?php } ?>
			  
			  
			  <?php } if($task_detail->task_activity_status!=2 && $task_detail->worker_agree!=1) {  ?>
                    <input type="submit" id="accept" name="accept" class="chbg fr" value="Send">
                    <?php } ?>
                    <div class="clear"></div>
                </div>
            </div>

			...
			echo anchor('message','<p class="alert"><strong>Alerts('.get_user_unread_notification().')</strong></p>','class="fpass"');
                ...
                foreach($allmessage as $res){
					...
					$act=$res->act;
						...
						switch ($act)
						{
							case 'newoffer': echo '<li>'.anchor('message/read/'.$res->message_id,'<p class="geo colgray fl"><img src="'.$user_image.'" alt="" width="40" height="40" class="fl marR5" />Ett bud har lagts på <strong class="colblue fsNorm">'.$res->task_name.'</strong> av <strong class="colblue fsNorm">'.$poster->first_name.' '.substr($poster->last_name,0,1).'</strong></p>','style="color:'.$color.'"').'<div class="clear"></div></li>'; 
							break;

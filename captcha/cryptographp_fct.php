...
// -----------------------------------------------
 ...
 echo $_SESSION['cryptdir']= dirname($cryptinstall);
 
 
 function dsp_crypt($cfg=0,$reload=1) {
 // Affiche le cryptogramme
 echo "<table><tr><td><img id='cryptogram' src='".base_url()."captcha/cryptographp.php?cfg=".$cfg."&".SID."'></td>";
 if ($reload) echo "<td><a title='".($reload==1?'':$reload)."' style=\"cursor:pointer\" onclick=\"javascript:document.images.cryptogram.src='".base_url()."captcha/cryptographp.php?cfg=".$cfg."&".SID."&'+Math.round(Math.random(0)*1000)+1\"><img src=\"".base_url()."captcha/images/reload.png\"></a></td>";
 echo "</tr></table>";
 }
